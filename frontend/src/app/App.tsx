...
function Avatar({ user, size = "md" }: { user: User; size?: "sm" | "md" | "lg" }) {
  ...
  return (
    <div
      className={`${dims} rounded-full flex items-center justify-center text-slate-200 font-semibold flex-shrink-0`}
...
function Badge({ children, color }: { children: React.ReactNode; color: "green" | "red" | "indigo" | "slate" }) {
  const styles = {
    ...
    indigo: { background: "#6366F115", color: "#6366F1", border: "1px solid #6366F130" },
      ...
      {cards.map((c, i) => (
        <Card key={i} className="flex flex-col gap-3">
          <div className="flex items-center justify-between">
            <span className="text-xs font-medium text-slate-200 uppercase tracking-wider">{c.label}</span>
          ...
          <div>
            ...
            <p className="text-xs text-slate-200 mt-1">{c.sub}</p>
          </div>
        </Card>
      ))}
    </div>
    ...
    <div className="flex items-center justify-between py-3 group hover:bg-slate-800/20 -mx-5 px-5 transition-colors duration-100 cursor-default">
      <div className="flex items-center gap-3">
        ...
        <p className="text-sm text-slate-200 ml-1">
          ...
          <span className="text-slate-200 mx-1.5">owes</span>
          <span className="font-semibold text-slate-200">
...
function Dashboard({
  balances, setBalances, activity, currentUser, onAddExpense, onRefresh,
}: {
  balances: Balance[];
  ...
  async function simplifyDebts() {
    await fetch("http://localhost:8080/api/simplify", { method: "POST" });
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-5">
        ...
        <Card noPad className="lg:col-span-3">
          <div className="flex items-center justify-between px-5 pt-5 pb-4">
            <div>
              <h2 className="text-sm font-semibold text-slate-200 font-['Outfit']">Balances</h2>
          ...
          <div className="px-5 pb-2">
            {balances.length === 0
              ? <p className="text-center text-slate-200 py-10 text-sm">All settled up!</p>
        ...
        <Card noPad className="lg:col-span-2">
          <div className="px-5 pt-5 pb-4">
            <h2 className="text-sm font-semibold text-slate-200 font-['Outfit']">Recent Activity</h2>
            <p className="text-xs text-slate-200 mt-0.5">Latest expenses</p>
          </div>
          <Divider />
            ...
            {activity.map((item, i) => (
              <div key={item.id}>
                <div className="flex items-start gap-3 py-3">
                  ...
                  <span className="text-xs font-semibold text-slate-200 tabular-nums flex-shrink-0 mt-0.5">
                    {fmt(item.amount)}
                  </span>
                </div>
      ...
      <button
        ...
        className="fixed bottom-7 right-7 flex items-center gap-2 px-4 py-3 rounded-xl text-sm font-semibold text-slate-200 transition-all duration-150 hover:opacity-90 active:scale-95"
...
function Input({
  ...
  return (
    <input
      ...
      className={`w-full px-3 py-2.5 rounded-lg text-sm text-slate-200 placeholder-slate-600 outline-none transition-colors ${right ? "text-right" : ""}`}
      style={{ background: "#0D1626", border: "1px solid #1E2D45" }}
...
function AddExpenseModal({
  ...
  async function handleSave() {
    ...
    if (splitType === "equal") {
      payloadParticipants = selectedUsers.map(u => u.id);
    } else if (splitType === "exact") {
      payloadParticipants = selectedUsers.map(u => ({ id: u.id, val: parseFloat(exactAmounts[u.id]) || 0 }));
    } else if (splitType === "percent") {
      payloadParticipants = selectedUsers.map(u => ({ id: u.id, val: parseFloat(percentAmounts[u.id]) || 0 }));
    }
    ...
    >
      ...
      >
        ...
        <div className="flex items-center justify-between px-6 py-4" style={{ borderBottom: "1px solid #1E2D45" }}>
          <h2 className="text-base font-semibold font-['Outfit'] text-slate-200">Add Expense</h2>
          <button
            onClick={onClose}
            className="h-7 w-7 rounded-lg flex items-center justify-center text-slate-200 hover:text-slate-200 hover:bg-slate-800 transition-colors"
          >
            <X size={14} />
          </button>
        </div>
        ...
        <div className="p-6 space-y-5 max-h-[78vh] overflow-y-auto">
          {/* Description */}
          <div>
            <label className="text-xs font-semibold text-slate-200 uppercase tracking-wider mb-1.5 block">Description</label>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="text-xs font-semibold text-slate-200 uppercase tracking-wider mb-1.5 block">Amount</label>
              <Input value={amount} onChange={setAmount} placeholder="0.00" type="number" />
            </div>
            <div className="relative">
              <label className="text-xs font-semibold text-slate-200 uppercase tracking-wider mb-1.5 block">Paid by</label>
              <button
              ...
              >
                ...
                <ChevronDown size={13} className="text-slate-200" />
              </button>
              {payerOpen && (
                <div
                ...
                >
                  {users.map(u => (
                    <button
                      ...
                      className="w-full flex items-center gap-2.5 px-3 py-2.5 text-sm text-slate-200 hover:bg-slate-800/40 transition-colors"
                    >
          ...
          {/* Participants */}
          <div>
            <label className="text-xs font-semibold text-slate-200 uppercase tracking-wider mb-2 block">Split Between</label>
          ...
          {/* Split type */}
          <div>
            <label className="text-xs font-semibold text-slate-200 uppercase tracking-wider mb-2 block">Split Method</label>
                ...
                {selectedUsers.map(u => (
                  <div key={u.id} className="flex items-center justify-between px-3 py-2 rounded-lg" style={{ background: "#0D1626", border: "1px solid #1E2D45" }}>
                    <div className="flex items-center gap-2">
                      <Avatar user={u} size="sm" />
                      <span className="text-sm text-slate-200">{u.name.split(" ")[0]}</span>
                    </div>
                    <span className="text-sm font-semibold text-slate-200 tabular-nums">{fmt(equalShare)}</span>
                  </div>
                ))}
              </div>
            )}
                  ...
                  <div key={u.id} className="flex items-center gap-3">
                    ...
                    <div className="flex items-center gap-1.5 w-28">
                      <Input
                        ...
                        right
                      />
                      <span className="text-slate-200 text-sm flex-shrink-0">%</span>
                    </div>
                  </div>
                ))}
                <div className="flex justify-between pt-1 text-xs">
        ...
        <div className="flex gap-2 px-6 py-4" style={{ borderTop: "1px solid #1E2D45" }}>
          <button
            onClick={onClose}
            className="flex-1 py-2.5 rounded-lg text-sm font-semibold text-slate-200 hover:text-slate-200 transition-colors"
            style={{ background: "#0D1626", border: "1px solid #1E2D45" }}
          >
            Cancel
          </button>
...
function FriendsScreen({
  users, onAddUser, onRemoveUser, balances, currentUser
}: {
  users: User[];
  ...
  return (
    <div className="space-y-5 max-w-xl">
      <Card>
        <h2 className="text-sm font-semibold text-slate-200 font-['Outfit'] mb-4">Add a Friend</h2>
        <div className="flex gap-2">
          <input
            ...
            className="flex-1 px-3 py-2.5 rounded-lg text-sm text-slate-200 placeholder-slate-600 outline-none"
            style={{ background: "#0D1626", border: "1px solid #1E2D45" }}
            onFocus={e => (e.target.style.borderColor = "#6366F1")}
            onBlur={e => (e.target.style.borderColor = "#1E2D45")}
          />
      ...
      <Card noPad>
        <div className="px-5 pt-5 pb-4 flex items-center justify-between">
          <h2 className="text-sm font-semibold text-slate-200 font-['Outfit']">Members</h2>
                ...
                <div className="flex items-center justify-between py-3.5">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-slate-200 mt-0.5 font-mono">{user.initials}</p>
                    </div>
                  </div>
                  {!isCurrentUser && (
                      ...
                      <div className="text-right">
                        {net === 0
                          ...
                          : <>
                              ...
                              <p className="text-xs text-slate-200 mt-0.5">{net > 0 ? "owes you" : "you owe"}</p>
                            </>
                        }
                      </div>
                      <button 
...
function ProfileSetup({ onSetProfile }: { onSetProfile: (name: string) => void }) {
  const [name, setName] = useState("");
  return (
    <div className="min-h-screen flex items-center justify-center p-4 font-['Inter']" style={{ background: "linear-gradient(135deg, #0A1120 0%, #0A1120 100%)" }}>
      <Card className="p-8 max-w-md w-full text-center" style={{ background: "#0D1626", border: "1px solid #1E2D45" }}>
        <h1 className="text-2xl font-bold font-['Outfit'] text-slate-200 mb-2">Welcome to FairShare</h1>
        <p className="text-slate-200 text-sm mb-6">Let's set up your profile.</p>
        <input 
          autoFocus
          className="w-full border border-[#1E2D45] rounded-xl px-4 py-3 text-slate-200 text-sm focus:outline-none focus:border-indigo-500 transition-colors mb-4 text-center"
          style={{ background: "#0D1626" }}
          placeholder="Enter your name" 
          value={name}
          onChange={e => setName(e.target.value)}
          onKeyDown={e => e.key === 'Enter' && name.trim() && onSetProfile(name.trim())}
        />
        <button 
          disabled={!name.trim()}
          onClick={() => onSetProfile(name.trim())}
          className="w-full py-3 rounded-xl text-sm font-semibold text-slate-200 transition-all disabled:opacity-50"
...
export default function App() {
  ...
  const [myUserId, setMyUserId] = useState<number | null>(() => {
    const saved = localStorage.getItem("fairshare_my_userid");
    return saved ? parseInt(saved, 10) : null;
  });

  const CURRENT_USER = users.find(u => u.id === myUserId) || { id: myUserId || 1, name: "Loading", initials: "LD", color: "#10B981" };
  ...
  async function fetchState() {
    try {
      ...
      if (myUserId && !mappedUsers.some((u: User) => u.id === myUserId)) {
        setMyUserId(null);
        localStorage.removeItem("fairshare_my_userid");
        setLoading(false);
        return;
      }

      const dRes = await fetch("http://localhost:8080/api/debts");
      const dData = await dRes.json();
      const mappedBalances = dData.map((d: any) => ({
        from: mappedUsers.find((u: User) => u.id === d.debtor_id) || { id: d.debtor_id, name: d.debtor_name, initials: getInitials(d.debtor_name), color: "#10B981" },
        to: mappedUsers.find((u: User) => u.id === d.creditor_id) || { id: d.creditor_id, name: d.creditor_name, initials: getInitials(d.creditor_name), color: "#10B981" },
        amount: d.amount
      }));
      setBalances(mappedBalances);

      const aRes = await fetch("http://localhost:8080/api/activity");
      const aData = await aRes.json();
      const mappedActivity = aData.map((a: any) => ({
        id: a.id,
        description: a.description,
        payer: mappedUsers.find((u: User) => u.id === a.payer_id) || { id: a.payer_id, name: "Unknown", initials: "??", color: "#10B981" },
        amount: a.amount,
        date: "Recently",
  ...
  async function handleSetProfile(name: string) {
    ...
    localStorage.setItem("fairshare_my_userid", newId.toString());
    setMyUserId(newId);
  }
  ...
  async function handleAddUser(name: string) {
    await fetch("http://localhost:8080/api/users", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ id: Date.now() % 10000, name })
    });
    fetchState();
  }
  ...
  if (!loading && !myUserId) {
    return <ProfileSetup onSetProfile={handleSetProfile} />;
  }

  if (loading && myUserId) {
    return <div className="min-h-screen flex items-center justify-center text-slate-200" style={{ background: "#0A1120" }}>Loading FairShare...</div>;
  }
    ...
    <div className="min-h-screen font-['Inter']" style={{ background: "#0A1120" }}>
      <div className="flex min-h-screen">
          ...
          <div className="px-5 h-14 flex items-center" style={{ borderBottom: "1px solid #1E2D45" }}>
            <div className="flex items-center gap-2">
              <div className="h-7 w-7 rounded-lg flex items-center justify-center" style={{ background: "#6366F1" }}>
                <Receipt size={14} className="text-slate-200" />
              </div>
              <span className="text-base font-bold font-['Outfit'] text-slate-200 tracking-tight">FairShare</span>
            </div>
          </div>
          ...
          <div className="px-3 py-3" style={{ borderBottom: "1px solid #1E2D45" }}>
            <div className="flex items-center gap-2.5 px-2 py-2 rounded-lg" style={{ background: "#131C2E" }}>
              ...
              <div>
                ...
                <p className="text-[10px] text-slate-200">Active session</p>
              </div>
            </div>
          </div>
        ...
        <main className="flex-1 min-w-0 flex flex-col">
          ...
          >
            <div>
              <h1 className="text-sm font-semibold font-['Outfit'] text-slate-200">
            ...
            <div className="flex gap-1 md:hidden">
              {NAV.map(item => (
                <button
                  ...
                  style={{ background: screen === item.id ? "#6366F118" : "transparent", color: screen === item.id ? "#6366F1" : "#475569" }}
                >
                  {item.icon}
                </button>
              ))}
